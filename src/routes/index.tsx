...
import { ArrowRight, Leaf, Award, Smile, Star, CheckCircle2, Loader2, Heart } from "lucide-react";
      ...
      <section className="bg-secondary/40 py-20">
        <div className="mx-auto grid max-w-6xl gap-10 px-6 md:grid-cols-3">
          <Promise
            ...
            body="Responsibly sourced and sustainably produced with love for the planet."
          />
          <Promise
            icon={Award}
          ...
          <Promise
            icon={Smile}
            title="Soft on Skin"
            body="Hypoallergenic and ultra-soft fabrics for the most sensitive skin."
          />
        </div>
      </section>
        ...
        <div className="flex flex-wrap justify-center gap-6">
          {categories.map((c) => (
            <Link
              ...
              className="group relative aspect-4/5 w-full overflow-hidden rounded-3xl bg-muted md:w-[calc(33.333%-1rem)]"
            >
              <img
                src={c.image || hero}
                alt={c.name}
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-background">
                ...
                <p className="text-sm opacity-90">{c.tagline || "Shop this collection"}</p>
              </div>
            </Link>
          ))}
        </div>
                  ...
                  return (
                    <div key={p.id} className={cn("group block", isOutOfStock && "opacity-80")}>
                        ...
                        <Link to="/product/$id" params={{ id: p.id }}>
                          <div className="relative aspect-square overflow-hidden rounded-2xl bg-card">
                            ...
                            <button
                              ...
                              className="absolute right-3 top-3 z-20 rounded-full bg-white/80 p-2 text-primary backdrop-blur-sm transition-all hover:bg-white hover:scale-110"
                            >
                              <Heart
                      ...
                      <div className="mt-4 flex items-baseline justify-between gap-2">
                        {isOutOfStock ? (
                          <h3 className="font-serif text-lg text-foreground/60 cursor-not-allowed">
                            {p.name}
                          </h3>
                        ) : (
                          <Link to="/product/$id" params={{ id: p.id }}>
